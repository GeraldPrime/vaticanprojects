...
def upload_form(request):
    ...
    return render(request, "user/forms_upload.html", {"recent_forms": recent_forms})
...
def add_estate_image(request):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"Image '{title}' added successfully to {dict(EstateImage.ESTATE_CHOICES)[estate]}.",
        )
        return redirect("estate_images_list")
...
def delete_estate_image(request):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"Image '{title}' from {estate_name} deleted successfully."
        )
